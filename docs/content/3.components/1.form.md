...
::tip
Need access to the underlying `UForm` methods? Use [`AutoFormPrimitive`](/components/form-primitive) which exposes the `form` reference directly via `formRef.form.submit()`.
::

## Related components

- [`AutoFormPrimitive`](/components/form-primitive) - Thin wrapper around `UForm` with auto-generated fields
- [`AutoFormModal`](/components/form-modal) - Modal with integrated form for best DX
